...
*Draft v3 — 2026-07-17. Incorporates Robert's review of v2. This document describes two things, deliberately kept separate: the design principles that have held since inception, and the current state of implementation — including where real growth has caused drift from that design. Drift is named, not hidden, and each instance is tied to its own tracked follow-up. See the end of this document for the current list.*
...
**Build (Guild).** The platform's own design-and-build coordination surface — specs, the build queue, an operations dashboard. This is where the multi-agent working model happens day to day: where a spec becomes a build item and status stays visible. Its scope is deliberately expanding in two directions: operations control, and *learning* — both from the platform's own mistakes (the German/Portuguese drift is a live example) and from the fast-moving landscape of new AI techniques and tools. In a domain area this dynamic, the build surface has to be a place where lessons accumulate, not just a task tracker.
...
|---|---|---|
| Curator | Mature — daily production since Feb 2026 | `--model=` flag regression; local fallback not wired into production path; Deep Dive script consolidation |
...
| Build (Guild) / CoS | Recently split; both in active redesign | CoS partner layer just starting (Python-only today, bounded OpenClaw shortly); accumulate-then-synthesize pattern not yet extended here |
...
The principle is evolving deliberately, not eroding: the intended next step is *bounded* agent-to-agent handoffs — starting with Chief of Staff coordinating with the build agent, CoS notifying the operator rather than the operator polling each agent, and the operator able to ask CoS anything and have it go find out across all agents. Control stays with the operator; what changes is that the operator gains a single point of contact instead of being the message bus between agents personally.
...
Same shape, five times. That repetition, proven rather than asserted, is the actual evidence for "platform over product."
...
```mermaid
flowchart TD
    A["RSS ~400/day + X Bookmarks ~332 + Web Search ≤20"] --> B[Source Trust Filter]
    B --> C[Haiku Pre-Filter]
    C --> D["Grok Daily Ranking (grok-4-1-fast-reasoning)"]
    D --> E["Top 20: Personalized Pool + Serendipity Pool"]
    E --> F[Mobile Site / Web Portal]
...
|---|---|---|---|
| Bulk pre-filter | Claude Haiku | ~$0.001/run | Yes |
| Daily ranking | `grok-4-1-fast-reasoning` | ~$0.314/run | Designed to be, currently broken — see below |
| Deep Dives | Claude Sonnet, on-demand | ~$0.25/session | Yes |
| AI Observations (daily) | Claude Haiku | ~$0.005–0.01/run | Yes |
| AI Observations (weekly) | Claude Sonnet, Sunday only | ~$0.07/week | Yes |

**Local capability — a core commitment with a current gap.** Curator was designed to run local-model-capable from the start, and part of it still does: `--dry-run` exercises the pipeline locally. But the audit found the documented local fallback tier (Ollama/Gemma) is not wired into the production scoring path as the March document described. The practical consequence is worth stating plainly: without a working local path, a cloud provider outage is a platform outage for that function, and cost exposure has no floor — the two things the local commitment exists to prevent. This is not a myth to strike from the record — it's a regression against a founding commitment: if cloud models become unavailable, the system is supposed to keep working locally, at zero cost. Restoring and verifying real local-model operation in Curator is part of the instrumented test plan with Claude Code (Follow-Up #1), not a deferred nice-to-have.

**AI Observations** is the platform's one working example of accumulate-then-synthesize done right: five distinct observation types, each on the model tier suited to its cost and complexity, synthesizing across a rolling 30-day baseline rather than reacting to a single day's data. The weekly Sonnet pass reads the accumulated history and asks, in effect, *what have I thought about this before* — a question none of the platform's other domains can currently answer about themselves. This is also where the thinking-record vision above stops being abstract: observation responses, deep-dive motivations, and reactions are exactly the raw material that record is made of.
...
CoS is also where Principle 5's planned evolution lands first: bounded agent-to-agent handoffs, starting with CoS coordinating with the build agent — CoS notifying the operator, and the operator able to ask CoS anything and have it find out across all agents, instead of personally being the message bus between them.
