# Spec #154: IoT Connect on mini-moi Production Hosting

**Supersedes:** `spec_154_connect_hq_minimoi_production_hosting_2026-09-03.md` (same specification, Connect HQ names retired per the IoT Connect naming decision of 2026-09-03, N3). Only names changed; the design, boundaries, and acceptance criteria are unchanged.
...
**First hosted release:** `iotconnect-v0.9.0-beta.1`
**Production URL:** `https://minimoi.ai/app/iotconnect`

## 1. Executive decision

Host the approved IoT Connect standalone beta on the existing mini-moi AWS
environment. IoT Connect is governed and presented by **Guild / Prototype Lab**,
...
IoT Connect receives:

- a Guild reference-demo entry with release and health information;
- an authenticated launch link at `/app/iotconnect`;
...
The standalone package proves that IoT Connect can start, stop, reset, report
...
application; IoT Connect is not rendered as a Guild template and does not share
Guild's application database.

### 3.2 Navigation

IoT Connect does not initially become a permanent top-level workspace beside
...
|---|---|
| Launch page | `https://minimoi.ai/app/iotconnect` |
| Presentation | `https://minimoi.ai/app/iotconnect/presentation` |
| Application Swagger | `https://minimoi.ai/app/iotconnect/docs` |
| Admin/workbench | `https://minimoi.ai/app/iotconnect/admin` |

All current IoT Connect pages and API routes must remain reachable under the
same `/app/iotconnect` prefix. The synthetic integration Swagger, integration
...
- Only the mini-moi owner can open `/app/iotconnect` in the first release.
...
its documents, or other mini-moi domains. If IoT Connect or a later prototype
...
4. IoT Connect maps the verified owner identity to its administrator and seeded
   demo-persona capabilities.
5. IoT Connect ignores or rejects browser-supplied demo-role headers.
...
The existing IoT Connect pages use absolute browser paths such as `/static`,
`/presentation`, and `/api/v1`. The shared mini-moi proxy rewrites many HTML,
CSS, and JavaScript URLs, but IoT Connect also declares an absolute JavaScript
...
- browser navigation, assets, API calls, redirects, downloads, Swagger, and
  OpenAPI all resolve beneath `/app/iotconnect`;
- the standalone local URLs remain unchanged;
- no IoT Connect call falls through to mini-moi's existing top-level `/api/*`
  passthrough; and
- no IoT Connect-specific exception is added to a broad top-level catch-all.
...
mini-moi portal (login, authorization, trusted identity)
   |
   +-- /app/iotconnect/* --> IoT Connect app :8095
                                  |
                                  +--> synthetic integrations :8096
                                  |
                                  +--> IoT Connect PostgreSQL :5432
```

Three services are deployed:

1. **IoT Connect app** — the only IoT Connect service proxied by the portal;
...
3. **IoT Connect PostgreSQL** — dedicated database and persistent volume,
...
- IoT Connect uses its own database, user, password, schema, and volume.
- Reset operations can affect only the IoT Connect database.
...
The existing shared `postgres-ai-agents` container is not used by IoT Connect.
...
The production Compose definition uses a IoT Connect-specific variable:

`IOTCONNECT_IMAGE_TAG=iotconnect-v0.9.0-beta.1`

It must not inherit `MINIMOI_IMAGE_TAG`. A normal push-to-main deployment may
refresh other mini-moi images but must leave the approved IoT Connect image and
...
IoT Connect receives a release workflow triggered only by:

- an approved `iotconnect-v*` Git tag; or
...
3. builds the IoT Connect runtime image;
4. pushes the immutable tag to `minimoi/iotconnect`;
5. confirms the image digest;
6. deploys only the IoT Connect services through SSM;
7. waits for app, integration, and database health; and
8. runs authenticated production smoke checks without resetting data.

The existing mini-moi deployment must not rebuild IoT Connect on unrelated main
branch changes. The IoT Connect deployment must not restart or replace unrelated
mini-moi services.
...
- promoted IoT Connect source for base-path and trusted-identity adaptation;
...
- SSM secret synchronization for the IoT Connect database credential;
- a IoT Connect-specific build/deploy workflow; and
...
- The candidate is promoted to
  `prototype-lab/projects/project-iot-connect/` without local/runtime files.
- The standalone verification suite passes from the tracked location.
- Git tag `iotconnect-v0.9.0-beta.1` identifies the exact approved source.
...
Through `https://minimoi.ai/app/iotconnect`, the owner can:
...
- Full IoT Connect stop/start retains state.
- Deterministic reset restores the seed state.
- A normal mini-moi deployment leaves the IoT Connect image, containers, and
  data unchanged except where Compose must re-evaluate an unchanged definition.
- A IoT Connect deployment leaves all unrelated mini-moi containers healthy and
...
2. Stop the IoT Connect app and integration services.
3. Leave the IoT Connect PostgreSQL volume intact.
4. Remove or disable the Guild launch entry and `/app/iotconnect` route if the
   backend is intentionally unavailable.
5. Confirm every pre-existing mini-moi service remains healthy.

The previous approved IoT Connect image remains in ECR. A later redeployment may
...
- redesign of the current IoT Connect demo; or
...
| P6 | IoT Connect declared an available Guild reference demo | Robert |
