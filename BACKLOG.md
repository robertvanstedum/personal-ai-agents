...
| B-014 | **BUG: Telegram delivery not working** | Curator Telegram delivery broken — symptom observed 2026-03-25. Root cause unknown, needs investigation. Check: bot token resolution (keychain vs env), `TELEGRAM_CHAT_ID` env var, message formatting changes, API response errors in logs. Start with `curator_server_stderr.log` after a run. |
