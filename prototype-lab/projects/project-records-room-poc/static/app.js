...
function renderEvent(event){const system=["session_opened","membership","moderator","state_change","document"].includes(event.kind);const node=element("article",undefined,`event event-${event.kind}${system?" event-system":""}`);node.id=`event-${event.id}`;const top=element("div",undefined,"event-top");top.append(element("span",event.actor_label[0],"avatar"),element("span",event.actor_label,"event-author"));if(!system)top.append(element("span",kindLabels[event.kind]||event.kind,"pill"));top.append(element("time",date(event.created),"event-time"));node.append(top,element("div",system?`${event.body} · ${event.actor_label}`:event.body,"event-body"));if(!system){let note=event.actor_kind==="agent"?"Authenticated client contribution · ":"";note+=`Record ${event.id.slice(0,8)}`;if(event.target)note+=` · to ${event.target}`;if(event.reference)note+=` · references ${event.reference.slice(0,8)}`;node.append(element("div",note,"event-note"));}
