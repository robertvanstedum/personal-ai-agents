#!/usr/bin/env python3
"""
domains/portuguese/dev_launcher.py — secret-free launchd entry point for
the host-native dev Portuguese service.

Same pattern as minimoi_portal/dev_launcher.py: DATABASE_URL only exists
in the project's gitignored .env, in Docker Compose form (host
"postgres"). Running natively on the Mac (outside Docker) needs that host
rewritten to "localhost" -- everything else about the value passes
through unchanged. Never prints, logs, or otherwise exposes the DSN; only
sets it in the environment of the child process it execs into.

launchd's ProgramArguments points here instead of directly at
html_server.py, so the local launchd plist itself never contains a
database password.

No shell involved anywhere (no `source`, no /bin/sh) -- the DSN's
password may contain shell-sensitive characters (quotes, $, backticks,
etc.), so this only ever does argv-list exec via os.execve, never a
shell string.
"""
...
# Defaults to the web app; pass an alternate script (e.g. leitura_rss.py,
# the scheduled ingestion job) as argv[1] to reuse this same secret-free
# launcher for it instead of duplicating the DSN-handling logic.
APP_SCRIPT = (
    Path(__file__).resolve().parent / sys.argv[1]
    if len(sys.argv) > 1
    else Path(__file__).resolve().parent / "html_server.py"
)
...
def main() -> None:
    ...
    if not APP_SCRIPT.is_file():
        _fail(f"Portuguese app script not found: {APP_SCRIPT}")
